...
def write_to_excel(file_name, df_merge_count, df_merge_number, df_merge_string, df_diff_count, df_diff_number,
                   df_diff_string):
    director_path = r"{CWD}\compare_folder".format(CWD=os.getcwd())
    Path(director_path).mkdir(parents=True, exist_ok=True)
    file_path = r"{CWD}\compare_folder\{table_name}.xlsx".format(table_name=file_name, CWD=os.getcwd())
...
def write_to_excel3(file_name, df_merge_number, df_merge_string, df_diff_number, df_diff_string):
    director_path = r"{CWD}\compare_folder_logic".format(CWD=os.getcwd())
    Path(director_path).mkdir(parents=True, exist_ok=True)
    file_path = r"{CWD}\compare_folder_logic\{table_name}.xlsx".format(table_name=file_name, CWD=os.getcwd())
    writer = pd.ExcelWriter(file_path)
    df_merge_number.to_excel(writer, 'CHECK_NUMBER', header=True, float_format="%.8f", index=None, encoding="utf8")
    df_merge_string.to_excel(writer, 'CHECK_STRING', header=True, float_format="%.8f", index=None, encoding="utf8")
    df_diff_number.to_excel(writer, 'NUMBER_ERROR', header=True, float_format="%.8f", index=None, encoding="utf8")
...
def compare_2_table(src_a, table_a, src_b, table_b):
    ...
    if (number_check and string_check):
    ...
    write_to_excel3(file_name, df_merge_number, df_merge_string, df_diff_number,
                   df_diff_string)
...
def run(list_task):
    # try:
    Parallel(n_jobs=5)(delayed(compare_2_table)(src_a, table_a, src_b, table_b) for
